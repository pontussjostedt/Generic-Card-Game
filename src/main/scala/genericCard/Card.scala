...
abstract class Card(
  ...
  val tags = scala.collection.mutable.Set.empty[Tag]
  updateCardImage()
  ...
  /** Draws the info image of the card
    ...
    *   absolute coordinates
    * @param width,height
    *   absolute values If there evere is a problem with performance remake me
    *   so that I create a bufferedImage at the start of the game
    */
  def drawInfo(
      g2d: java.awt.Graphics2D,
      x: Int,
